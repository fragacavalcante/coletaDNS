...
def consulta_dns(rede, ip, mascara, fqdn, tipo_busca, lock):
    ...
    for i in range(quantidade_ips):
        ...
        try:            
            ...
            grava_informacoes_dns(rede, ip_definido, "ok", lock) # ENVIA VARIAVEL LOCK QUE SERÁ UTILIZADO PARA BLOQUEAR O ACESSO AO ARQUIVO
        except dns.resolver.NXDOMAIN:           
##            print (u"Consulta a um DNS publico sobre uma zona ou fqdn que não existe")          
            grava_informacoes_dns(rede, ip_definido, "10", lock)            
...
##            print (u"Consulta a um DNS privado sobre uma zona ou fqdn para qual le não é autoritativo")           
            grava_informacoes_dns(rede, ip_definido, "30", lock)           
...
def grava_informacoes_dns(rede, ip_definido, resposta,lock):
    lock.acquire()
    nome_arquivo = 'resultado_consulta_ips_' + 'rede_' + str(rede) + '.txt'
    ...
    resultado_consuta_ips.writelines(resposta)
    resultado_consuta_ips.writelines("\n")    
